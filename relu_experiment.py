...
for run in range(num_runs):
    ...
    # ModelBRunNetworks = [deepcopy_network(modelRescale)]
    ...
    for epoch in range(num_epochs):
        for i, (images, labels) in enumerate(train_loader):
            ...
            if (i+1) % 150 == 0:
                ...
                ModelARunNetworks.append(deepcopy_network(modelDefault))
                # ModelBRunNetworks.append(deepcopy_network(modelRescale))
                ModelCRunNetworks.append(deepcopy_network(modelReinit))

    FileLog.add_runlog(runlogA)
    # FileLog.add_runlog(runlogB)
    FileLog.add_runlog(runlogC)
    ModelANetworks.append(ModelARunNetworks)
    # ModelBNetworks.append(ModelBRunNetworks)
    ModelCNetworks.append(ModelCRunNetworks)

FileLog.save(experiment_number)
#torch.save([ModelANetworks, ModelBNetworks, ModelCNetworks],str(experiment_number)+'.pt')
torch.save([ModelANetworks, ModelCNetworks],str(experiment_number)+'.pt')
